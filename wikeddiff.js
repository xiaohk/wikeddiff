...
export var WikEdDiff = function () {
  ...
  this.config.regExp = {
    ...
    split: {
      ...
      paragraph: new RegExp(
        '(\\r\\n|\\n|\\r){2,}|[' + this.config.regExpNewParagraph + ']',
        'g'
      ),

      // Split into lines
      line: new RegExp(
        '\\r\\n|\\n|\\r|[' + this.config.regExpNewLinesAll + ']',
        'g'
      ),
    ...
    slideBorder: new RegExp('[' + this.config.regExpBlanks + ']$'),
    ...
    clipBlank: new RegExp('[' + this.config.regExpBlanks + ']+', 'g'),
  ...
  this.config.blockHandler = function (event, element, type) {
        ...
        if (container !== null) {
          ...
          for (var i = 0; i < spansLength; i++) {
            if (spans[i] !== block && spans[i] !== mark) {
              if (
                spans[i].className.indexOf(' wikEdDiffBlockHighlight') !== -1
              ) {
                spans[i].className = spans[i].className.replace(
                  / wikEdDiffBlockHighlight/g,
                  ''
                );
              } else if (
                spans[i].className.indexOf(' wikEdDiffMarkHighlight') !== -1
              ) {
                spans[i].className = spans[i].className.replace(
                  / wikEdDiffMarkHighlight/g,
                  ''
                );
              }
            }
    ...
    if (type === 'click') {
      ...
      if (window.getComputedStyle !== undefined) {
        line = parseInt(
          window.getComputedStyle(corrElement).getPropertyValue('line-height')
        );
      }
  ...
  this.init = function () {
    ...
    if (this.config.showBlockMoves === true) {
      ...
      if (typeof GM_info === 'object') {
        var script =
          'var wikEdDiffBlockHandler = ' +
          this.config.blockHandler.toString() +
          ';';
          ...
          if (newToken.length !== oldToken.length) {
            ...
            if (left + right !== shorterToken.length) {
              ...
              if (longerToken.indexOf(shorterToken) === -1) {
                ...
                if (
                  left < shorterToken.length / 2 &&
                  right < shorterToken.length / 2
                ) {
  ...
  this.slideGaps = function (text, textLinked) {
    ...
    while (i !== null) {
      ...
      // Find gap end
        ...
        var back = gapBack;
        var gapFrontBlankTest = regExpSlideBorder.test(
        ...
        if (text.tokens[back].link === null) {
          while (
            ...
            if (front !== null) {
              ...
              if (
                regExpSlideBorder.test(text.tokens[front].token) !==
                gapFrontBlankTest
              ) {
                frontStop = front;
              }
  ...
  this.calculateDiff = function (
    ...
    if (newStart === undefined) {
      newStart = this.newText.first;
    }
    if (oldStart === undefined) {
      oldStart = this.oldText.first;
    }
    if (up === undefined) {
      up = false;
    }
    ...
    while (i !== null) {
      if (this.newText.tokens[i].link === null) {
        ...
        if (
          Object.prototype.hasOwnProperty.call(symbols.hashTable, token) ===
          false
        ) {
          ...
          if (
            ...
            if (recursionLevel === 0) {
              ...
              if (level === 'character') {
                ...
                ).concat(token.match(this.config.regExp.countChunks) || []);
  ...
  this.setFixed = function () {
    ...
    for (var section = 0; section < sectionsLength; section++) {
      ...
      for (var i = 0; i < maxPathLength; i++) {
        ...
        // Mark fixed blocks
        for (
          var block = groups[group].blockStart;
          block <= groups[group].blockEnd;
          block++
        ) {
    ...
    for (var block = 0; block < blocksOldLength; block++) {
      ...
      // Move after closest previous fixed block
        ...
        for (var fixed = block; fixed >= 0; fixed--) {
          if (
            blocksOld[fixed].type === '=' &&
            blocksOld[fixed].fixed === true
          ) {
  ...
  this.insertMarks = function () {
    ...
    for (var moved = 0; moved < groupsLength; moved++) {
      ...
      // Find closest fixed block to the left
        ...
        for (
          var fixed = lookupSorted[movedGroup.blockStart] - 1;
          fixed >= 0;
          fixed--
        ) {
          if (
  ...
  this.clipDiffFragments = function () {
    ...
    for (var fragment = 0; fragment < fragmentsLength; fragment++) {
      ...
      var headingsEnd = [];
      while (
        (regExpMatch = this.config.regExp.clipHeading.exec(text)) !== null
      ) {
      ...
      var lastIndex = null;
      while (
        (regExpMatch = this.config.regExp.clipParagraph.exec(text)) !== null
      ) {
      ...
      if (fragment !== 2) {
        ...
        if (rangeLeft === null) {
          ...
          for (var j = 0; j < linesLength; j++) {
            if (
              lines[j] > this.config.clipLineLeftMax ||
              lines[j] > rangeLeftMax
            ) {
              break;
            }
        ...
        if (rangeLeft === null) {
          ...
          if (
            (regExpMatch = this.config.regExp.clipBlank.exec(text)) !== null
          ) {
            if (
      ...
      if (fragment !== fragments.length - 3) {
        ...
        if (rangeRight === null) {
          for (var j = paragraphs.length - 1; j >= 0; j--) {
            ...
            if (
              paragraphs[j] <
              textLength - this.config.clipParagraphRightMin
            ) {
        ...
        if (rangeRight === null) {
          ...
          while (
            ...
            if (
              regExpMatch.index >
              textLength - this.config.clipBlankRightMin
            ) {
              if (lastPos !== null) {
                rangeRight = lastPos;
      ...
      if (rangeLeft !== null) {
        ...
        textLeft = textLeft.replace(
          this.config.regExp.clipTrimNewLinesLeft,
          ''
        );
        ...
        if (rangeLeftType === 'chars') {
          omittedLeft = '~';
          textLeft = textLeft.replace(
            this.config.regExp.clipTrimBlanksLeft,
            ''
          );
      ...
      if (rangeRight !== null) {
        ...
        textRight = textRight.replace(
          this.config.regExp.clipTrimNewLinesRight,
          ''
        );
        ...
        if (rangeRightType === 'chars') {
          omittedRight = '~';
          textRight = textRight.replace(
            this.config.regExp.clipTrimBlanksRight,
            ''
          );
          ...
          if (this.config.showBlockMoves === false || version === 'old') {
            ...
            if (version === 'old') {
              if (this.config.coloredBlocks === true) {
                html =
                  this.htmlCustomize(
                    this.config.htmlCode.blockColoredStart,
                    color
                  ) +
                  text +
  ...
  this.unitTests = function () {
    ...
    if (diff !== text) {
      ...
      console.log(
        'OK: wikEdDiff unit test passed: diff consistent with new text.'
      );
    }
    ...
    if (diff !== text) {
      ...
      console.log(
        'OK: wikEdDiff unit test passed: diff consistent with old text.'
      );
    }
  ...
  this.debugShortenText = function (text, max, end) {
    ...
    if (text.length > max) {
      text =
        text.substr(0, max - 1 - end) + '…' + text.substr(text.length - end);
    }
    return '"' + text + '"';
  };
      ...
      for (var i = 0; i < timerLength; i++) {
        console.log(
          text +
            ...
            i +
            ': ' +
            this.recursionTimer[i].toFixed(2) +
            ' ms'
        );
      }
    }
